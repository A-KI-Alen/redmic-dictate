...
class ConfigTests(unittest.TestCase):
    def test_default_config_matches_hotkey_plan(self) -> None:
        ...
        self.assertEqual(config.openai_realtime_prompt, "")
        self.assertEqual(config.openai_realtime_audio_rate, 24000)
        self.assertEqual(config.openai_realtime_commit_seconds, 2.0)
        self.assertEqual(config.openai_realtime_finish_timeout_seconds, 7.0)
        self.assertEqual(config.openai_realtime_mini_transcribe_eur_per_minute, 0.0028)
        self.assertEqual(config.openai_realtime_transcribe_eur_per_minute, 0.0056)
        self.assertEqual(config.openai_usage_admin_key_env, "OPENAI_ADMIN_KEY")
        self.assertEqual(config.openai_usage_poll_delay_seconds, 20.0)
        self.assertEqual(config.openai_usage_poll_attempts, 3)
